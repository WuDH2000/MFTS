...
from src.training.stinfor_dataset import EnvSepDataset as Dataset
import time
    ...
    with torch.no_grad():
        for batch_idx, (mixed, gt_env_points, gt_wav) in \
            ...
            for bb in range(mixed.shape[0]):
                ...
                label_wav_s_b = gt_wav[bb] #[1, maxns, len, 3]
                ...
                label_wav_s_b = label_wav_s_b[exist_tensor]
            ...
            label_wav_s = torch.cat(label_wav_s, axis = 0)
            ...
            est_wav = model_ws([mixed_s, est_env_points_s]) #[B, 4, len]

            loss = network_ws.loss(est_wav, label_wav_s)
            metrics_batch = network_ws.metrics(est_wav.detach(), label_wav_s.detach())
...
def train_epoch(model_rs: nn.Module, device: torch.device,
    ...
    with tqdm(total=len(train_loader), desc='Train', ncols=140) as t:
        for batch_idx, (mixed, gt_env_points, gt_wav) in enumerate(train_loader):
            mixed = mixed.to(device).float()
            # print(mixed.dtype)
            gt_wav = gt_wav.to(device).float() #[B, ns, 4, tt]
            gt_env_points = gt_env_points.to(device).float()
            # print('wav', gt_wav.shape) #[B, ns, len, 3]
            ...
            with torch.no_grad():
                ...
                for bb in range(mixed.shape[0]):
                    ...
                    label_wav_s_b = gt_wav[bb] #[maxns, 4, len]
                    # print(label_wav_s_b.shape)
                    exist_tensor = ((torch.max(est_env_points_s_b, axis = -1)[0] > 0.25) 
                                            & (torch.max(tgt_env_points_s_b, axis = -1)[0] > 0.25))
                    # print(exist_tensor.shape)
                    est_env_points_s_b = est_env_points_s_b[exist_tensor]
                    ...
                    tgt_env_points_s_b = tgt_env_points_s_b[sel_idx:sel_idx + 1]
                    label_wav_s_b = label_wav_s_b[sel_idx]   #[4, len]
                ...
                est_env_points_s = torch.cat(est_env_points_s, axis = 0) #[B, 1, T]
                tgt_env_points_s = torch.cat(tgt_env_points_s, axis = 0)
                label_wav_s = torch.cat(label_wav_s, axis = 0) #[B, 4, len]
                mixed_s = torch.cat(mixed_s, axis = 0)
                
            est_wav = model_ws([mixed_s, est_env_points_s.detach()]) #[B, 4, len]
            ...
            # Gradient clipping
            torch.nn.utils.clip_grad_norm_(model_ws.parameters(), 0.5)
...
def train(args: argparse.Namespace):
    ...
    if use_cuda and data_parallel:
        ...
        model_ws = nn.DataParallel(model_ws, device_ids=device_ids)
    ...
    optimizer = network_ws.optimizer(model_ws, **args.optim, data_parallel=data_parallel)
    ...
    if args.start_epoch > 0:
        ...
        _, train_metrics, val_metrics = utils.load_checkpoint(
            checkpoint_path, model_ws, optim=optimizer, lr_sched=lr_scheduler,
    ...
    try:
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            ...
            curr_train_metrics = train_epoch(model_rs, device, optimizer,
                                             train_loader, model_ws)
            #raise KeyboardInterrupt
            curr_test_metrics = test_epoch(model_rs, device, val_loader, model_ws)
            # LR scheduler
            ...
            utils.save_checkpoint(
                checkpoint_file, epoch, model_ws, optimizer, lr_scheduler,
                train_metrics, val_metrics, data_parallel)
            logging.info("Saved checkpoint at %s" % checkpoint_file)
                # curr_test_metrics = test_epoch(model_rs, device, test_loader, model_ws)
...
if __name__ == '__main__':
    ...
    exec("import %s as network_ws" % args.model_ws)
    logging.info("Imported the model_ws from '%s'." % args.model_ws)
