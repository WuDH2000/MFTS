...
class STFT(nn.Module):
    def __init__(self, n_fft: int, n_hop: int, win_len: Optional[int] = None, win: str = 'hann_window') -> None:
        ...
        if win == 'hann_window':
            ...
            window = torch.sqrt(torch.hann_window(self.win_len))
...
if __name__ == '__main__':
    x = torch.randn((8, 32, 16000))
    Fx = torch.stft(x.reshape(8*32, 16000), n_fft=256, hop_length=128, win_length=256, return_complex=True)#, center=False)
    print(Fx.shape)
    stft = STFT(**paras_16k)
    X, ol = stft.stft(x)
    x_p = stft.istft(X, ol)
