...
model_rs_path = 'experiments/trans_noinfor/spatialnet_nsrc2_mc/22.pt'
...
with torch.no_grad():
    for batch_idx, (mixed, clean_wav, traj_t, _, T60) in \
        ...
        _, est_wav = network_rs.loss(est_wav, clean_wav, return_est=True)
        # est_sch_wav = est_sch_wav[:, 0]
        # clean_wav = clean_wav[:, 0]

        get_snr = snr(est_wav[:, :, 0], clean_wav[:, :, 0]).mean(dim = -1).reshape(-1, 1).cpu().detach().numpy()
        rec_schsnr.append(get_snr)
        print('schsnr', get_snr)
        get_sdr = sdr(est_wav[:, :, 0], clean_wav[:, :, 0]).mean(dim = -1).reshape(-1, 1).cpu().detach().numpy()
        rec_schsdr.append(get_sdr)
        print('schsdr', get_sdr)
        get_sisnr = si_snr(est_wav[:, :, 0], clean_wav[:, :, 0]).mean(dim = -1).reshape(-1, 1).cpu().detach().numpy()
        ...
        get_snr = torch.mean(snr(est_wav, clean_wav), dim = [-1, -2]).reshape(-1, 1).cpu().detach().numpy()
        rec_snr.append(get_snr)
        print('snr', get_snr)
        get_sdr = torch.mean(sdr(est_wav, clean_wav), dim = [-1, -2]).reshape(-1, 1).cpu().detach().numpy()
        rec_sdr.append(get_sdr)
        print('sdr', get_sdr)
        get_sisnr = torch.mean(si_snr(est_wav, clean_wav), dim = [-1, -2]).reshape(-1, 1).cpu().detach().numpy()
        ...
        B = est_wav.shape[0]
        est_sch_wav_frame = est_wav[:, :, 0].reshape(B, 2, 125, -1)
...
print('sisnr', np.mean(get_schsisnr))
print('snr', np.mean(get_schsnr))
print('sdr', np.mean(get_schsdr))

print('sisnr', np.mean(get_sisnr))
print('snr', np.mean(get_snr))
print('sdr', np.mean(get_sdr))
