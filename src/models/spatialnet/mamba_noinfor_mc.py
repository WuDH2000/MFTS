...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        for m in self.layers:
            x, _ = m(x)
        out = self.decoder(x) #[B, F, T, 2 * 2M]
        
        if not torch.is_complex(out):
            out = torch.view_as_complex(out.float().reshape(B, F, T, -1, 2))  # [B,F,T,Spk*M]
            

        out = out.permute(0, 3, 1, 2) #[B, spk * M, F, T]
        out = self.stft.istft(out, stft_paras) #[B, spk * 4, 48000]
        
        out = out.reshape(B, 2, 4, -1) #[B, 2, M, 48000]
...
#             torch.mean(-snr(pred, tgt), dim = (-1)) * 0.9)#[batch]
def mix_loss(pred, tgt):
    # pred[batch, M, 48000]
    # print('loss sisnr', torch.mean(si_snr(pred, tgt), dim = (-1)))
    # print('loss snr', torch.mean(snr(pred, tgt), dim = (-1)))
    return torch.mean(-snr(pred, tgt), dim = (-1))
...
def metrics(output, gt):
    ...
    metrics['si0'] = [sisnr_metrics(output[:, :, 0:1], gt[:, :, 0:1]).cpu().detach().numpy()]
    metrics['snr0'] = [snr_metrics(output[:, :, 0:1], gt[:, :, 0:1]).cpu().detach().numpy()]
    return metrics
...
if __name__ == '__main__':
    ...
    z = model(x)
    print(z.shape)
    label = torch.rand(8, 2, 4, 48000).cuda()
