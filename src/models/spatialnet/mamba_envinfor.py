...
class Net(nn.Module):
    def __init__(
        ...
        self.spat_encoder = nn.Conv1d(in_channels=1, out_channels=dim_hidden, 
                                 kernel_size=encoder_kernel_size, stride=1, padding="same")
        full = None
        layers = []
...
if __name__ == '__main__':
    ...
    x = torch.randn(2, 4, 48000).cuda()
    spat_inf = torch.randn(2, 1, 376).cuda()
    # print(x.shape)
    model = Net().cuda()
    ...
    label = torch.rand(2, 4, 48000).cuda()
