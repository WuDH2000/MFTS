...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.stft.istft(out, stft_paras) #[B, ns, 48000]
...
def metrics(output, gt):
    ...
    metrics = {}
    metrics['sch_sisnr'] = [si_snr(output, gt).cpu().detach().numpy().mean()]
    metrics['sch_snr'] = [snr(output, gt).cpu().detach().numpy().mean()]
    return metrics
...
if __name__ == '__main__':
    ...
    label = torch.rand(8, 1, 48000).cuda()
