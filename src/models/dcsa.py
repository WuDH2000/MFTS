...
from src.models.dcc_tf_stinfor import mod_pad
...
class Net(nn.Module):
    def __init__(self, L=32,
                 ...
                 proj=True, lookahead=True, loc_onsing = True, **kwargs):
        ...
        if loc_onsing:
            self.in_conv = nn.Sequential(
                nn.Conv1d(in_channels=num_mics + 1,
                        out_channels=enc_dim, kernel_size=kernel_size, stride=L,
                        padding=0, bias = False),
                nn.ReLU())
        else:
            self.in_conv = nn.Sequential(
                nn.Conv1d(in_channels=num_mics * 2 + 1,
                        out_channels=enc_dim, kernel_size=kernel_size, stride=L,
                        padding=0, bias = False),
                nn.ReLU())
        ...
        self.out_conv_t = nn.Sequential(
            nn.ConvTranspose1d(
                in_channels=enc_dim, out_channels=3,
                kernel_size=3 * L,
                stride=L,
                padding=L, bias=False),
            nn.Tanh())
        # self.out_conv_s = nn.Sequential(
        #     nn.Conv2d(
...
def loss(pred, tgt, est_env):
    # print(pred.shape) #[batch, 3, 48000]
    # print(tgt.shape)  #[B, 3, 48000]
    # print(est_env.shape)
    # loss_doa = F.mse_loss(pred[:, :3], tgt[:, :3])
    loss_diff = 0
    step = 1
    while step < 2 ** 11:
        tloss = F.mse_loss(pred[:, step::step] - pred[:, 0:-step:step], 
                       tgt[:, step::step] - tgt[:, 0:-step:step])
        loss_diff += tloss
        step *= 2
    # getloss = 0.5 * loss_doa + 0.5 * loss_diff
    getloss = F.mse_loss(pred * est_env.permute(0, 2, 1), est_env.permute(0, 2, 1) * tgt) * 0.5 + loss_diff * 0.5
    # print(getloss)
    # getloss = F.mse_loss(pred * est_env, est_env * tgt)
    # print('loss_doa {}/{}={}'.format(torch.mean(loss_doa), torch.mean(loss), torch.mean(loss_doa)/torch.mean(loss)))
    # print('loss_diff {}/{}={}'.format(torch.mean(loss_diff), torch.mean(loss), torch.mean(loss_diff)/torch.mean(loss)))
    # print('loss_en {}/{}={}'.format(torch.mean(loss_en), torch.mean(loss), torch.mean(loss_en)/torch.mean(loss)))
    return getloss
...
def angle_diff(pred, tgt, get_norm, tgt_norm, keepbatch):
    # print(pred.shape)  #[B, 48000, 3]
    # print(tgt.shape)  #[B, 48000, 3]
    # get_norm: [B, 1, 48000]
    pred = cart2sph(pred)  #[16, 16000, 3] ->[0, ele, azi]
    tgt = cart2sph(tgt)
    res_a = torch.pow(angular_error(pred[..., 1], 
            pred[..., 2], tgt[..., 1], tgt[..., 2]), 2) #[B, 48000]
    res_a = res_a.unsqueeze(1) #[B, 1, 48000]
    if not keepbatch:
        w_res_pred = (torch.sqrt((torch.sum(res_a * (get_norm ** 2), axis = -1) 
                        / torch.sum(get_norm ** 2, axis = -1))).squeeze(-1) / torch.pi * 180).mean()
        w_res_tgt = (torch.sqrt((torch.sum(res_a * (tgt_norm ** 2), axis = -1) 
                        / torch.sum((tgt_norm) ** 2, axis = -1))).squeeze(-1) / torch.pi * 180).mean()
    else:
        w_res_pred = (torch.sqrt((torch.sum(res_a * (get_norm ** 2), axis = -1) 
                        / torch.sum(get_norm ** 2, axis = -1))).squeeze(-1) / torch.pi * 180)
        w_res_tgt = (torch.sqrt((torch.sum(res_a * (tgt_norm ** 2), axis = -1) 
                        / torch.sum((tgt_norm) ** 2, axis = -1))).squeeze(-1) / torch.pi * 180)
    return w_res_pred, w_res_tgt


def metrics(output, gt, pred_norm, tgt_norm, keepbatch = False):
    """ Function to compute metrics """
    metrics = {}
    adp, adt = angle_diff(output.cpu(), gt.cpu(), pred_norm.cpu(), tgt_norm.cpu(), keepbatch)
    metrics['adp'] = [adp.cpu().detach().numpy()]
    metrics['adt'] = [adt.cpu().detach().numpy()]
    # print(angle_diff(output.cpu(), gt.cpu()).shape)
    return metrics

if __name__ == '__main__':
    model = Net().cuda()
    x = torch.randn(8, 5, 48000).cuda()
    z = model(x)
    print(z.shape)
    
    tgt_traj = torch.randn(8, 48000, 3).cuda()
    tgt_norm = torch.randn(8, 1, 48000).cuda()
    getloss = loss(z, tgt_traj, tgt_norm)
    getmetrics = metrics(z, tgt_traj, tgt_norm, tgt_norm)
    print(getloss)
    print(getmetrics)